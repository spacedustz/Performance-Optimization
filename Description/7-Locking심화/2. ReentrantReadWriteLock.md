...
주로 읽기 작업이 많은 어플리케이션에서 읽기 작업이 느리거나 읽기 작업에 드는 비용이 크며, 읽기 작업을 수행하는 Reader Thread가 여러개일때, 

1개의 Thread가 공유 Resource를 읽을때 다른 Reader Thread를 Blocking 시킬때 앱의 성능이 문제가 생깁니다.

위같은 사례들을 ReentrantReadWriteLock을 사용하여 Reader들의 **공유 리소스 동시 접근**이 가능하게 하여 해결하기 위한 클래스가 ReentrantReadWriteLock 입니다.
