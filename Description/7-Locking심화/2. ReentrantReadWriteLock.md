...
주로 읽기 작업이 많은 어플리케이션에서 읽기 작업이 느리거나, 읽기 작업에 드는 비용이 클때,

각 Reader Thread에서 공유 Resource를 읽는걸 Blocking 시키면 앱의 성능이 문제가 생깁니다.

위같은 사례들을 해결하기 위한 클래스가 ReentrantReadWriteLock 입니다.
...
처음은 Read/Write Lock과 비교하기 위해 일반 ReentrantLock을 사용해서 차이점을 확인하기 위해 기본 ReentrantLock을 사용합니다.
...
main 함수를 만들어서랜덤한 값을 이용해 Writer / Reader Thread를 만들어 시간을 측정하여 테스트 해보겠습니다.

Reader Thread는 실행 순서를 join()으로 제어해 각 스레드가 완료될떄마다 수행될 수 있게 합니다.
...
이번엔 각 Reader Thread들이 동시에 실행되지 않고 1개의 Reader가 Lock을 잡고있을떄 Blocking 될 수 있도록 ReadWriteLock을 사용해 보겠습니다.

기존 ReentrantLock을 사용했을 땐, 1개의 Thread가 Lock을 들고있으면 다른 Thread는 Blocking 되었던 반면,

ReadWriteLock을 사용하여 Reader Thread 들의 **공유 리소스 동시 접근**이 가능하게 변경하였습니다.
